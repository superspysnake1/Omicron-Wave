function SIR_Omicron_Fitting
  ...
  a = 0.04; %time sick
  b = 0.0000003; %transmissibility
  suscep_init = 7600000.0;
  infec_init = qdata(1);

  function dy = model_1(t,y,k) % DE
    a = k(1); % Assignes the parameters in the DE the current value of the parameters
    b = k(2);
    dy = zeros(2,1); %Initializes dy array
    dy(1) = - b * y(1) * y(2); %Susceptible equation
    dy(2) = b * y(1) * y(2) - a * y(2); %Infected equation
  end

  function error_in_data = moder(k) % computing the error in the data
    [T, Y] = ode23s(@(t,y)(model_1(t,y,k)),tforward,[suscep_init, infec_init]);
    % solves the DE; output is written in T and Y

    q=Y(tmeasure(:),2); %Assigns the y-coordinates of the solution at the 
  ...
  k0 = [a b]; %Main routine; assigns initial values of parameters
  [T, Y] = ode23s(@(t,y)(model_1(t,y,k0)),tforward,[suscep_init, infec_init]);
...
yint = Y(tmeasure(:),2); %Computing the y-coordinates corresponding to 
                         %the tdata

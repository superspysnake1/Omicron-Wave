function SVIR_Omicron_Fitting
  ...
  a = 0.132469417181062; %recovery rate
  b = 0.000000033411580; %transmissibility
  p = 0.000000000014374; %vaccination rate
  d = 0.638094251416886; %transmissibility prob if vaccinated
  suscep_init = 2300000.0;
  vacc_init = 5300000.0;
  infec_init = qdata(1);
  ...
  function dy = model_1(t,y,k) % DE
    ...
    dy(1) = -p*y(1) - b*y(1)*y(3); %Susceptible equation
    dy(2) = p*y(1) - d*b*y(2)*y(3); %Vaccinated equation
    dy(3) = b*y(1)*y(3)+d*b*y(2)*y(3)-a*y(3); %Infected equation
  end

  function error_in_data = moder(k) % computing the error in the data
    [T, Y] = ode23s(@(t,y)(model_1(t,y,k)),tforward,[suscep_init, vacc_init, infec_init]);
  ...
  k0 = [a b p d]; %Main routine; assigns initial values of parameters
  [T, Y] = ode23s(@(t,y)(model_1(t,y,k0)),tforward,[suscep_init, vacc_init, infec_init]);
  %Solves the DE with the initial values of the parameters
  yint = Y(tmeasure(:),3);
  %Assigns the y-coordinates of the solution at tdata to yint
  
  %Generate plot with data and regression based only on initial parameter values. 
  figure(1)
  subplot(1,2,1);
  plot(tdata,qdata,'r.');
  hold on
